...
## Top-Level Structure
1. The workspace is defined in `Cargo.toml` and contains 4 crates: `nvr`, `ffmpeg-bus`, `nvr-db`, and `nvr-dashboard`.
...
### 1. nvr (main application)
- Entry point: `nvr/src/main.rs`
...
- API assembly: `nvr/src/api.rs`, mounting `/user`, `/pipe`, `/system`, and `/nvr`.
- Management layer: `nvr/src/manager.rs`, using a global `RwLock<HashMap<String, Arc<Pipe>>>` to manage pipeline lifecycles.
- Media pipeline: `nvr/src/media/pipe.rs`, converting business config into ffmpeg-bus input/output and running async processing.
...
## Core Runtime Flow
1. Client sends `POST /pipe/add` (`nvr/src/handler/media_pipe.rs`).
